            ...
            if (detailResp.IsSuccessStatusCode)
            {
                ...
                if (result?.status == "success" && result.data != null)
                {
                    MainThread.BeginInvokeOnMainThread(() =>
                    {
                        ...
                        if (result.data.stockOpnameResultList != null && result.data.stockOpnameResultList.Count > 0)
                        {
                            foreach (var opr in result.data.stockOpnameResultList)
                            {
                                ...
                                grid.Add(new Label { Text = opr.transDate ?? "-", TextColor = Color.FromArgb("#616363"), FontSize = 12, HorizontalOptions = LayoutOptions.End }, 1, 0); 

                                // Row 1
                                grid.Add(new Label { Text = "Stocker", TextColor = Color.FromArgb("#616363"), FontSize = 14 }, 0, 1);
                                grid.Add(new Label { Text = opr.fullname ?? "-", TextColor = Color.FromArgb("#191c1d"), FontSize = 14 }, 1, 1); 

                                // Row 2
                                grid.Add(new Label { Text = "Total Pemeriksaan", TextColor = Color.FromArgb("#616363"), FontSize = 14 }, 0, 2);
                                grid.Add(new Label { Text = $"{opr.countDetail} Items", TextColor = Color.FromArgb("#191c1d"), FontSize = 14, FontAttributes = FontAttributes.Bold }, 1, 2);
